...
<body>
    ...
    async function createRemote(){ var name=document.getElementById('remote-name').value; var url=document.getElementById('remote-url').value; var rt=document.getElementById('remote-token').value; await localApi('/drive/remotes',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,url:url,token:rt})}); toast('分布节点已添加'); await loadDistributedData(); }
