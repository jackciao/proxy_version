...
func (s *CamouflageService) createNginxConfig(domain string) error {
	...
	// Container-internal paths (mapped from host via volumes)
...
server {
    ...
    # SSL Configuration
    ssl_certificate /usr/local/openresty/nginx/conf/ssl/%s/fullchain.pem;
    ssl_certificate_key /usr/local/openresty/nginx/conf/ssl/%s/privkey.pem;
    ...
    access_log /usr/local/openresty/nginx/logs/%s.access.log;
    error_log /usr/local/openresty/nginx/logs/%s.error.log;
}
...
func (s *CamouflageService) reloadOpenResty() error {
	...
	testCmd := exec.Command("docker", "exec", s.containerName, "openresty", "-t")
	testOutput, err := testCmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("配置测试失败: %s", string(testOutput))
	}

	// Reload
	reloadCmd := exec.Command("docker", "exec", s.containerName, "openresty", "-s", "reload")
	reloadOutput, err := reloadCmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("重载失败: %s", string(reloadOutput))
	}
