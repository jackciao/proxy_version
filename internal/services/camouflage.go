...
func (s *CamouflageService) deploySiteHTML(domain string) error {
	for _, siteRoot := range s.siteRootHostPaths(domain) {
		...
		if err := deployCamouflageIcons(siteRoot); err != nil {
			return err
		}
	}
...
func deployCamouflageIcons(siteRoot string) error {
	iconData, err := os.ReadFile(camouflageIconSourcePath())
	if err != nil {
		return fmt.Errorf("读取网盘图标失败: %v", err)
	}

	for _, name := range []string{"icon.png", "favicon.png", "apple-touch-icon.png"} {
		if err := os.WriteFile(filepath.Join(siteRoot, name), iconData, 0644); err != nil {
			return err
		}
	}

	return os.WriteFile(filepath.Join(siteRoot, "site.webmanifest"), []byte(streamVaultWebManifest), 0644)
}

func camouflageIconSourcePath() string {
	for _, path := range []string{
		filepath.Join(".", "image.png"),
		"/app/image.png",
		"/opt/proxy_version/image.png",
	} {
		if pathExists(path) {
			return path
		}
	}
	return "/app/image.png"
}
...
const streamVaultWebManifest = `{
  "name": "Gotee 网盘",
  "short_name": "Gotee",
  "description": "Gotee 私人网盘",
  "start_url": "/drive.html",
  "scope": "/",
  "display": "standalone",
...
<body>
    <div class="app">
        <aside class="rail">
            ...
            <button class="rail-btn" data-pane="distributed"><span>⌁</span>分布</button>
        ...
        <aside class="left">
            ...
            <section class="quota">
                ...
                <div class="bar" id="quota-bar"></div>
                <div class="legend"><span><i class="dot" style="background:#f43f5e"></i>音频</span><span><i class="dot" style="background:#22c55e"></i>文档</span><span><i class="dot" style="background:#3b82f6"></i>图片</span><span><i class="dot" style="background:#8b5cf6"></i>视频</span><span><i class="dot" style="background:#94a3b8"></i>其他</span></div>
    ...
    function currentToken(){ return activeRemote ? activeRemote.token : localToken; }
    function remoteApiBase(remote){ var u=String(remote.url||'').replace(/\/+$/,''); return u.endsWith('/api')?u:u+'/api'; }
    function activeApiBase(){ return activeRemote ? remoteApiBase(activeRemote) : apiBase; }
    function authHeaders(extra){ var h = extra || {}; h.Authorization = 'Bearer ' + currentToken(); return h; }
    async function parseJSONResponse(res){ var data=null; var text=await res.text(); if(text){ try{data=JSON.parse(text);}catch(e){data={raw:text};} } return data || {}; }
    async function api(path, opts){
        opts = opts || {};
        opts.headers = opts.headers || {};
        opts.headers.Authorization = 'Bearer ' + currentToken();
        if(activeRemote){
            var remoteRes = await fetch(activeApiBase() + path, opts);
            var remoteData = await parseJSONResponse(remoteRes);
            if(remoteRes.status===401) throw new Error('远端 Token 无效或已过期');
            if(!remoteRes.ok) throw new Error((remoteData && remoteData.error) || '远端请求失败');
            return remoteData;
        }
        var lastErr;
    ...
    async function localApi(path, opts){ var saved=activeRemote; activeRemote=null; try{return await api(path,opts);} finally{activeRemote=saved;} }
    ...
    function quotaCategory(f){ var cat=fileCategory(f.mime,f.name); if(cat==='audio'||cat==='image'||cat==='video')return cat; if(cat==='doc'||cat==='pdf')return 'doc'; return 'other'; }
    function renderQuota(){ var used=usedBytes(); var total=quotaBytes(); document.getElementById('quota-text').textContent=(activeRemote?activeRemote.name+' · ':'')+'已用 '+fmtBytes(used)+' / '+fmtBytes(total); var sums={audio:0,doc:0,image:0,video:0,other:0}; files.forEach(function(f){ if(!f.folder){sums[quotaCategory(f)]+=Number(f.size)||0;} }); var bar=document.getElementById('quota-bar'); var html=''; ['audio','doc','image','video','other'].forEach(function(k){ var w=total>0?Math.min(100,sums[k]/total*100):0; if(w>0) html+='<span class="seg '+k+'" style="width:'+w.toFixed(3)+'%" title="'+k+' '+fmtBytes(sums[k])+'"></span>'; }); bar.innerHTML=html; document.getElementById('quota-input').value=quotaGB; }
    function renderCrumb(){ var crumb=document.getElementById('crumb'); if(pane==='distributed'){crumb.innerHTML='<span class="cr current">分布存储</span>';return;} if(section==='recent'){crumb.innerHTML='<span class="cr current">最近上传</span>';return;} if(section==='starred'){crumb.innerHTML='<span class="cr current">星标文件</span>';return;} if(section==='trash'){crumb.innerHTML='<span class="cr current">回收站</span>';return;} if(pane==='album'){crumb.innerHTML='<span class="cr current">相册</span>';return;} if(pane==='video'){crumb.innerHTML='<span class="cr current">视频</span>';return;} var html='<span class="cr" data-i="-1">'+(activeRemote?escapeHTML(activeRemote.name):'根目录')+'</span>'; for(var i=0;i<cwd.length;i++) html+='<span class="sep">/</span><span class="cr '+(i===cwd.length-1?'current':'')+'" data-i="'+i+'">'+escapeHTML(cwd[i].name)+'</span>'; crumb.innerHTML=html; crumb.querySelectorAll('.cr').forEach(function(el){ el.onclick=function(){ var idx=parseInt(el.getAttribute('data-i'),10); cwd=idx===-1?[]:cwd.slice(0,idx+1); renderAll(); }; }); }
    ...
    function isVisualFile(f){ var cat=fileCategory(f.mime,f.name); return cat==='image'||cat==='video'; }
    function itemVisualHTML(f,cat,large){ if(f.folder)return '<span class="file-icon folder">夹</span>'; if(isVisualFile(f))return '<span class="thumb-wrap" data-thumb="'+f.id+'"><span class="file-icon '+cat+'">'+iconLabel(cat)+'</span></span>'; return '<span class="file-icon '+cat+'">'+iconLabel(cat)+'</span>'; }
    function cardVisualHTML(f,cat){ if(f.folder)return '<div class="file-icon folder">夹</div>'; if(isVisualFile(f))return '<div class="card-thumb '+cat+'" data-thumb="'+f.id+'"><span class="file-icon '+cat+'">'+iconLabel(cat)+'</span></div>'; return '<div class="file-icon '+cat+'">'+iconLabel(cat)+'</div>'; }
    function hydrateThumbs(root){ root=root||document; root.querySelectorAll('[data-thumb]').forEach(function(node){ var id=node.getAttribute('data-thumb'); var f=findItem(id); if(!f||node.getAttribute('data-ready'))return; node.setAttribute('data-ready','1'); fetchBlob(id,false).then(function(blob){ var url=trackObjectURL(URL.createObjectURL(blob)); var cat=fileCategory(f.mime,f.name); if(node.classList.contains('card-thumb')){ if(cat==='image'){ var img=document.createElement('img'); img.className='card-thumb'; img.src=url; img.alt=f.name; node.replaceWith(img); } else { var v=document.createElement('video'); v.className='card-thumb video'; v.src=url; v.muted=true; v.preload='metadata'; v.playsInline=true; node.replaceWith(v); } return; } if(cat==='image'){ var img=document.createElement('img'); img.className='thumb'; img.src=url; img.alt=f.name; node.innerHTML=''; node.appendChild(img); } else { var vid=document.createElement('video'); vid.className='thumb'; vid.src=url; vid.muted=true; vid.preload='metadata'; vid.playsInline=true; node.innerHTML=''; node.appendChild(vid); } }).catch(function(){}); }); }
    function renderList(){ var content=document.getElementById('content'); var data=currentList(); document.getElementById('trash-clear').classList.toggle('hidden',section!=='trash'||!files.some(function(f){return f.trashed;})); if(!data.length){ var msg=section==='trash'?'回收站为空':(query?'没有匹配的文件':'此文件夹为空，点击上传按钮开始添加文件'); content.innerHTML='<div class="empty"><div>'+msg+'</div></div>'; return; } if(view==='grid'){ var cards=''; data.forEach(function(f){ var cat=f.folder?'folder':fileCategory(f.mime,f.name); cards+='<div class="card" data-id="'+f.id+'">'+cardVisualHTML(f,cat)+'<div class="cname">'+escapeHTML(f.name)+(f.starred?' <span class="star">★</span>':'')+'</div><div class="csub">'+(f.folder?'文件夹':fmtBytes(f.size))+'</div></div>'; }); content.innerHTML='<div class="grid">'+cards+'</div>'; } else { var folderCount=data.filter(function(f){return f.folder;}).length; var rows=''; data.forEach(function(f){ var cat=f.folder?'folder':fileCategory(f.mime,f.name); rows+='<tr data-id="'+f.id+'" class="'+(f.id===selectedId?'selected':'')+'"><td><div class="name">'+itemVisualHTML(f,cat,false)+'<span class="name-text">'+escapeHTML(f.name)+(f.starred?'<span class="star">★</span>':'')+'</span></div></td><td>'+(f.folder?'-':fmtBytes(f.size))+'</td><td>'+typeLabel(f)+'</td><td>'+dateOnly(f.updated_at||f.created_at)+'</td><td style="text-align:right;padding-right:6px"><button class="row-action" data-menu="1" data-id="'+f.id+'" title="更多操作">⋯</button></td></tr>'; }); content.innerHTML='<table class="table"><thead><tr><th style="width:46%">文件名 <span style="color:#9aa3af">'+folderCount+' 个文件夹，'+(data.length-folderCount)+' 个文件</span></th><th style="width:14%">大小</th><th style="width:14%">类型</th><th style="width:18%">修改时间</th><th style="width:8%;text-align:right;padding-right:6px">操作</th></tr></thead><tbody>'+rows+'</tbody></table>'; } bindContentEvents(); hydrateThumbs(content); }
    function renderMedia(kind){ var content=document.getElementById('content'); document.getElementById('trash-clear').classList.add('hidden'); var data=files.filter(function(f){return !f.folder&&!f.trashed&&fileCategory(f.mime,f.name)===kind;}); data.sort(function(a,b){return b.uploadedAt-a.uploadedAt;}); if(query){ var q=query.toLowerCase(); data=data.filter(function(f){return f.name.toLowerCase().indexOf(q)!==-1;}); } if(!data.length){ content.innerHTML='<div class="empty"><div>'+(kind==='image'?'相册暂无照片':'视频列表暂无内容')+'</div><div class="sub">上传对应文件后即可在此查看</div></div>'; return; } if(view==='list'){ var rows=''; data.forEach(function(f){ rows+='<tr data-id="'+f.id+'"><td><div class="name"><span class="thumb-wrap" data-thumb="'+f.id+'"><span class="file-icon '+kind+'">'+iconLabel(kind)+'</span></span><span class="name-text">'+escapeHTML(f.name)+'</span></div></td><td>'+fmtBytes(f.size)+'</td><td>'+typeLabel(f)+'</td><td>'+dateOnly(f.updated_at||f.created_at)+'</td><td style="text-align:right;padding-right:6px"><button class="row-action" data-menu="1" data-id="'+f.id+'" title="更多操作">⋯</button></td></tr>'; }); content.innerHTML='<table class="table"><thead><tr><th style="width:46%">名称</th><th style="width:14%">大小</th><th style="width:14%">类型</th><th style="width:18%">上传时间</th><th style="width:8%;text-align:right;padding-right:6px">操作</th></tr></thead><tbody>'+rows+'</tbody></table>'; bindContentEvents(); hydrateThumbs(content); return; } var html='<div class="album">'; data.forEach(function(f){ html+='<div class="photo '+kind+'-tile" data-id="'+f.id+'">'+(kind==='image'?'<img alt="'+escapeHTML(f.name)+'" loading="lazy">':'<video preload="metadata" muted playsinline></video><div class="fallback">'+escapeHTML((f.name.split('.').pop()||'').toUpperCase())+'</div>')+'<div class="pmeta">'+escapeHTML(f.name)+' · '+fmtBytes(f.size)+'</div></div>'; }); html+='</div>'; content.innerHTML=html; content.querySelectorAll('.photo').forEach(function(node){ var id=node.getAttribute('data-id'); node.onclick=function(){openPreview(id);}; fetchBlob(id,false).then(function(blob){ if(!blob)return; var url=trackObjectURL(URL.createObjectURL(blob)); var el=node.querySelector(kind==='image'?'img':'video'); if(el)el.src=url; }).catch(function(){}); }); }
    function renderDistributed(){ document.getElementById('trash-clear').classList.add('hidden'); var content=document.getElementById('content'); content.innerHTML='<div class="dist-wrap"><section class="dist-panel"><h3>当前网盘 API Token</h3><div class="dist-token" id="api-token-box">加载中...</div><button class="btn" id="copy-api-token" style="margin-top:10px">复制 Token</button><h3 style="margin-top:18px">添加远端网盘</h3><div class="dist-form"><input id="remote-name" placeholder="节点名称，例如：日本节点"><input id="remote-url" placeholder="地址，例如：https://jp.example.com:8443"><input id="remote-token" placeholder="远端 API Token"><button class="btn primary" id="remote-add">添加节点</button></div></section><section class="dist-panel"><h3>分布节点</h3><div id="remote-list"><div class="empty" style="height:180px">加载中...</div></div></section></div>'; bindDistributedEvents(); loadDistributedData().catch(function(e){toast(e.message||'加载分布节点失败');}); }
    function renderRemoteList(){ var box=document.getElementById('remote-list'); if(!box)return; var html='<div class="remote-row"><div><div class="remote-name">本机网盘 '+(!activeRemote?'<span class="active-node">当前</span>':'')+'</div><div class="remote-url">'+location.origin+'</div></div><div class="remote-actions"><button class="btn" data-local-node="1">切换</button></div></div>'; remotes.forEach(function(r){ html+='<div class="remote-row"><div><div class="remote-name">'+escapeHTML(r.name)+' '+(activeRemote&&String(activeRemote.id)===String(r.id)?'<span class="active-node">当前</span>':'')+'</div><div class="remote-url">'+escapeHTML(r.url)+'</div></div><div class="remote-actions"><button class="btn" data-remote-switch="'+r.id+'">切换</button><button class="btn danger" data-remote-del="'+r.id+'">删除</button></div></div>'; }); box.innerHTML=html; box.querySelectorAll('[data-local-node]').forEach(function(b){b.onclick=function(){activeRemote=null;cwd=[];section='all';pane='storage';loadState().catch(function(e){toast(e.message||'切换失败');});};}); box.querySelectorAll('[data-remote-switch]').forEach(function(b){b.onclick=function(){var id=b.getAttribute('data-remote-switch');activeRemote=remotes.filter(function(r){return String(r.id)===String(id);})[0]||null;cwd=[];section='all';pane='storage';loadState().catch(function(e){toast(e.message||'远端连接失败');});};}); box.querySelectorAll('[data-remote-del]').forEach(function(b){b.onclick=function(){deleteRemote(b.getAttribute('data-remote-del'));};}); }
    async function loadDistributedData(){ var t=await localApi('/drive/api-token'); driveAPIToken=t.token||''; var tokenBox=document.getElementById('api-token-box'); if(tokenBox)tokenBox.textContent=driveAPIToken; var data=await localApi('/drive/remotes'); remotes=data.remotes||[]; renderRemoteList(); }
    function bindDistributedEvents(){ var add=document.getElementById('remote-add'); if(add)add.onclick=function(){createRemote().catch(function(e){toast(e.message||'添加失败');});}; var copy=document.getElementById('copy-api-token'); if(copy)copy.onclick=function(){ if(navigator.clipboard&&driveAPIToken)navigator.clipboard.writeText(driveAPIToken); toast('Token 已复制'); }; }
    async function createRemote(){ var name=document.getElementById('remote-name').value; var url=document.getElementById('remote-url').value; var rt=document.getElementById('remote-token').value; var result=await localApi('/drive/remotes',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,url:url,token:rt})}); toast(result.warning || (result.reciprocal_added ? '分布节点已双向添加' : '分布节点已添加')); await loadDistributedData(); }
    async function deleteRemote(id){ if(!confirm('确定删除这个分布节点？'))return; await localApi('/drive/remotes/'+encodeURIComponent(id),{method:'DELETE'}); if(activeRemote&&String(activeRemote.id)===String(id))activeRemote=null; await loadDistributedData(); toast('分布节点已删除'); }
    function renderAll(){ renderQuota(); renderCrumb(); closeRowMenu(); if(pane==='distributed') renderDistributed(); else if(pane==='album') renderMedia('image'); else if(pane==='video') renderMedia('video'); else renderList(); updateProps(); document.querySelectorAll('.rail-btn').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-pane')===pane);}); document.querySelectorAll('.quick-item').forEach(function(q){q.classList.toggle('active',q.getAttribute('data-section')===section);}); var allow=pane==='storage'&&section==='all'; document.getElementById('upload-btn').disabled=!allow; document.getElementById('folder-btn').disabled=!allow; }
    ...
    function findUploadTask(id){ return uploadTasks.filter(function(t){return t.id===id;})[0]||null; }
    function notifyUploadIdle(){ if(uploadActiveTasks().length===0){ var failed=uploadTasks.filter(function(t){return t.status==='failed';}).length; var pending=uploadTasks.filter(function(t){return t.status==='paused'||t.status==='waiting';}).length; if(failed)toast('上传队列有 '+failed+' 个失败任务'); else if(!pending&&uploadTasks.some(function(t){return t.status==='done';}))toast('上传队列已完成'); } }
    function startUploadTask(task){ if(!task||task.status==='canceled'||task.status==='done')return; task.loaded=0; task.progress=0; task.speed=0; task.error=''; task.status='uploading'; task.abortReason=''; renderQueue(); var xhr=new XMLHttpRequest(); task.xhr=xhr; var lastLoaded=0; var lastAt=Date.now(); xhr.open('POST',activeApiBase()+'/drive/upload',true); xhr.setRequestHeader('Authorization','Bearer '+currentToken()); xhr.upload.onprogress=function(e){ if(e.lengthComputable){ var now=Date.now(); var dt=Math.max((now-lastAt)/1000,.001); task.loaded=e.loaded; task.progress=e.total>0?e.loaded/e.total*100:0; task.speed=Math.max(0,(e.loaded-lastLoaded)/dt); lastLoaded=e.loaded; lastAt=now; renderQueue(); } }; xhr.onload=function(){ task.xhr=null; var data=null; try{data=xhr.responseText?JSON.parse(xhr.responseText):null;}catch(e){} if(xhr.status===401){ task.status='failed'; task.error=activeRemote?'远端 Token 无效':'登录已过期'; } else if(xhr.status>=200&&xhr.status<300){ task.loaded=task.size; task.progress=100; task.status='done'; task.speed=0; loadState().catch(function(){}); } else { task.status='failed'; task.error=(data&&data.error)||'上传失败'; task.speed=0; } renderQueue(); notifyUploadIdle(); };
        xhr.onerror=function(){ task.xhr=null; task.status='failed'; task.error='网络异常'; task.speed=0; renderQueue(); notifyUploadIdle(); };
        xhr.onabort=function(){ task.xhr=null; task.speed=0; if(task.abortReason==='pause'){task.status='paused';task.error='';} else {task.status='canceled';task.error='';task.loaded=0;task.progress=0;} renderQueue(); notifyUploadIdle(); };
        var fd=new FormData(); fd.append('parent',task.parent); fd.append('files',task.file); xhr.send(fd); }
    function runUploadAction(id,act){ var task=findUploadTask(id); if(!task)return; if(act==='pause'&&(task.status==='uploading'||task.status==='waiting')){ task.abortReason='pause'; if(task.xhr)task.xhr.abort(); else {task.status='paused';renderQueue();} } else if(act==='resume'&&(task.status==='paused'||task.status==='failed')){ startUploadTask(task); } else if(act==='cancel'){ task.abortReason='cancel'; if(task.xhr)task.xhr.abort(); else {task.status='canceled';task.loaded=0;task.progress=0;renderQueue();} } }
    ...
    async function handleUpload(list){ var arr=Array.prototype.slice.call(list); if(!arr.length)return; var parent=currentParent(); openQueueModal(); arr.forEach(function(file){ var task={id:'up_'+(++uploadTaskSeq),name:file.name,size:file.size,file:file,parent:parent,loaded:0,progress:0,speed:0,status:'waiting',error:'',xhr:null,abortReason:''}; uploadTasks.unshift(task); startUploadTask(task); }); renderQueue(); toast('已加入上传队列'); }
    ...
    async function fetchBlob(id,download){ var res=await fetch(activeApiBase()+'/drive/files/'+encodeURIComponent(id)+(download?'/download':''),{headers:authHeaders({})}); if(res.status===401){ if(activeRemote)throw new Error('远端 Token 无效'); localStorage.removeItem('cloud_token');location.href='/login.html';throw new Error('登录已过期');} if(!res.ok)throw new Error('文件数据不可用'); return await res.blob(); }
